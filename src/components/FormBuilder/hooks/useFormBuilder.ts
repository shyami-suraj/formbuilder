...
const useFormBuilder = (props: useFormBuilderProps) => {
  ...
  const editControlProperties = (item: FormLayoutComponentChildrenType) => {
    ...
    formContainer.children.forEach((cont, index) => {
      if (cont.id === item.id) {
        const newChildren = formContainer.children.slice();
  ...
  const moveControlFromSide = (
    ...
    const currentItemContainer = componentsCopy.filter((con, index) => {
      return con.container.id === item.containerId;
    })[0];

    const moveItemToContainer = componentsCopy.filter((con, index) => {
  ...
  const moveControl = (
    ...
    if (dragIndex !== undefined && item.id) {
      if (item.containerId === containerId) {
        const formContainer = componentsCopy.filter((con, index) => {
          return con.container.id === containerId;
        })[0];
  ...
  const publishForm = () => {
    ...
    dispatch(saveTemplate(currentTemplate)).unwrap().then((newTemplate:any)=>{
  ...
  const saveForm = () => {
    ...
    dispatch(saveTemplate(currentTemplate)).unwrap().then((resolvedvalue:any)=>{
      showModalStrip(
        "success",
